...
test("Field validations", function () {
    ...
    ok(input.form.find('#messagearea').css('display') === 'block');
    ok(input.form.find('#messagearea').text() === 'Error: After N occurrences field must be a positive integer value (max. 1000)');
    ...
    // None of the Repeat on checkboxes selected
    input.form.find('#riweeklyweekdays input:checkbox').attr('checked', false);
    input.form.find('.rirefreshbutton').click();
    ok(input.form.find('#messagearea').css('display') === 'block');
